...
Vagrant.configure("2") do |config|
  ...
  # config.vm ... <= parametres globaux pour toutes les machines
  config.vm.define 'server' do |machine|
    # machine.vm ... <= parametres pour la machine specifique

    # ici on configure le nom de la machine
    machine.vm.hostname = 'server'

    # ici on configure le script de provisioning
    machine.vm.provision 'shell', path: 'provision.server.sh'

    # on force une typologie de reseau et une adresse IP (et peut etre une
    # carte reseau virtuelle)
